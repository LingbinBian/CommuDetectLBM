...
%      n_s: SNR
%      vari: degree of inter-individual variation
%      hrf_ind: 1: add hrf, 2: do not add hrf
%   Output:
%      Timeseries: a struct containing the Gaussian time series
%
% Version 1.0
...
ab_sub=cell(N_sub,1);
if vari==0   % if there is no inter-individual variation about labels
    for n=1:N_sub
        latent_seg_sub{n,1}=latent_seg;
        ab_sub{n,1}=ab_seg;
    end
    
else   % if there is inter-individual variation about labels
...
for n=1:N_sub
    if hrf_ind==1
        subdir=fullfile(data_path,['Data/synthetic/','DIIV',num2str(vari),'_hrf','/n',num2str(n_s)],num2str(s_id(n)));
    else
        subdir=fullfile(data_path,['Data/synthetic/','DIIV',num2str(vari),'/n',num2str(n_s)],num2str(s_id(n)));
    end    
    cd(subdir);

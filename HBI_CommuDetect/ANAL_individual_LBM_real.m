...
parfor s=1:N_subj
    fprintf('subject: %d\n',s)
    for ds=1:4
      [z_g{s,ds},Q(s,ds)] = CommuDetectLBM(group_adj{s,1}{1,c(ds)},K_max,nu,rho,kappa_sq);   
...
for i=1:N_subj
        ...
        label_temporal(:,1)=z_g{i,1};
        label_temporal(:,2)=z_g{i,2};
        label_temporal=labelswitch(label_temporal);
        z_g{i,1}=label_temporal(:,1);
        z_g{i,2}=label_temporal(:,2);
        half_1(i,1)=nmi(z_g{i,1},z_g{i,2});
        label_temporal=zeros(N,2);
end
   
for i=1:N_subj
        label_temporal=zeros(N,2);
        label_temporal(:,1)=z_g{i,3};
        label_temporal(:,2)=z_g{i,4};
        label_temporal=labelswitch(label_temporal);
        z_g{i,3}=label_temporal(:,1);
        z_g{i,4}=label_temporal(:,2);
        half_2(i,1)=nmi(z_g{i,3},z_g{i,4});
        label_temporal=zeros(N,2);
end
...
% Schaefer's atlas
% c1=0.67921    nu=3.2337     rho=0.92278   kappa_sq=14.185       K_max=24
% c2=0.67942    nu=6.4209     rho=0.98480   kappa_sq=12.56       K_max=16
% c3=0.71607    nu=2.8727     rho=0.99576   kappa_sq=3.0498       K_max=23
% c4=0.57478    nu=2.1091     rho=0.11939   kappa_sq=15.919       K_max=5
% c5=    nu=     rho=   kappa_sq=       K_max=
% c6=    nu=     rho=   kappa_sq=       K_max=

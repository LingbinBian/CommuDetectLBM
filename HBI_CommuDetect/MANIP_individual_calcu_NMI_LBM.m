...
% using latent block model (individual-level analysis)
%
% Version 1.0 
...
N_sub=100;
N_state=3;
load(['Data/synthetic/','DIIV',num2str(DIIV),'/n',num2str(n_s),'/1001','/timeseries.mat']);
...
NMI_indi_LBM=zeros(N_sub,N_state);

for i=1:N_sub
    for j=1:N_state
        NMI_indi_LBM(i,j)=nmi(z_g{i,j},true_latent_sub{i,1}(:,j));
    end
end

data_path = fileparts(mfilename('fullpath'));
NMI_indi_path=fullfile(data_path,['Results/synthetic_LBM/','DIIV',num2str(DIIV),'/n',num2str(n_s),'/NMI_indi_LBM']);
save(NMI_indi_path,'NMI_indi_LBM')

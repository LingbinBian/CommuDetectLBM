...
length_nmi=(N_node*N_node-N_node)/2;
...
set(gcf,'unit','centimeters','position',[6 10 8 12])
set(gca,'Position',[.22 .28 .7 .6]);

x=[1,2];

[h,p_v,ci,stats] = vartest2(data1,data2);
if p_v < 0.05
    [h1,p,ci1] = ttest2(data1,data2,'Vartype','unequal');
else
    [h1,p,ci1] = ttest2(data1,data2);
end
sigline(x,max(max([data1,data2])),p,p_v); %
x=x+1;
...
results_path=fullfile(data_path,'Results/real_LBM/LR/between_subjects_2b0b');
save(results_path); 
saveas(gcf,'Results/real_LBM/LR/between_subjects_2b0b.fig')
saveas(gcf,'Results/real_LBM/LR/between_subjects_2b0b.svg')
% -------------------------------------------------------------------------
% sigline

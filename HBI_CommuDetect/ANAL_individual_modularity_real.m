% Community detection using modularity (real fMRI data)
...
% 
if atlas==1
    load(['Results/real_Bayes_opt_M/combination',num2str(c_comb),'.mat'])
    gamma_M=bestGamma;  % gamma
else
    load(['Results/real_Kong_Bayes_opt_M/combination',num2str(c_comb),'.mat'])
    gamma_M=bestGamma;  % gamma
end
...
for s=1:N_subj
    fprintf('subject: %d\n',s)
    for ds=1:N_state
      [z_g{s,ds},Q(s,ds)] = modularity_und(group_adj{s,1}{1,ds},gamma_M);   

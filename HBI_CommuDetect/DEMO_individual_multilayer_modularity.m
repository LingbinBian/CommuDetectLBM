% Community detection using multilayer modularity
...
elseif datatype==0
   ...
   % coupling parameter
    ...
    vari=30; % true community structure
    hrf_ind=0;
...
for ds=1:N_state
    [communities(:,:,ds),Q_multi(1,ds)] = multilayer_modularity(group_adj_3D(ds,:), gamma, omega);
end
...
for s=1:N_subj
    ...
    for ds=1:N_state
        ...
        K_g(s,ds)=length(unique(z_g{s,ds}));
    end
end
...
if datatype==0
   ...
   group_path=fullfile(data_path,['Results/synthetic_multilayer_modularity_coupling',num2str(omega),'/','DIIV',num2str(vari),'/n',num2str(n_s),'/',num2str(gamma),'/grouplevel_data']);
  
   save(group_path);
elseif datatype==1
   if atlas==1 
       if session_n==1
           group_path=fullfile(data_path,['Results/real_multilayer_modularity_coupling',num2str(omega),'/',num2str(gamma),'/LR/','grouplevel_data']);
       elseif session_n==2
           group_path=fullfile(data_path,['Results/real_multilayer_modularity_coupling',num2str(omega),'/',num2str(gamma),'/RL/','grouplevel_data']);
       end
   else
       if session_n==1
           group_path=fullfile(data_path,['Results/real_Kong_multilayer_modularity_coupling',num2str(omega),'/',num2str(gamma),'/LR/','grouplevel_data']);
       elseif session_n==2
           group_path=fullfile(data_path,['Results/real_Kong_multilayer_modularity_coupling',num2str(omega),'/',num2str(gamma),'/RL/','grouplevel_data']);
       end
   end
   save(group_path);    
end
